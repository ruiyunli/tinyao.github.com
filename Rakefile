...
task :post do
  ...
  eng = ENV["eng"]
  slug = title  #title.downcase.strip.gsub(' ', '-').gsub(/[^\w-]/, '')
  begin
  ...
  if File.exist?(filename)
    if ask("#{filename} already exists. Do you want to overwrite?", ['y', 'n']) == 'n'
      abort("rake aborted!")
    else
      stime = Time.now.strftime("%H:%M:%S")
      filename = File.join(CONFIG['posts'], "#{date}-#{stime}-#{slug}.#{CONFIG['post_ext']}")
    end
  end
